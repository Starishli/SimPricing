...
class EuropeanOption(SimBase):
    def __init__(self, sim_engine, sim_times):
        super().__init__(sim_engine, sim_times)

    def _calc_payoff(self, underlying_prc, sim_t, **kwargs):
        strike = kwargs["strike"]
        prc = max(underlying_prc - strike, 0) * np.exp(-self.r * sim_t)
        return prc
...
class AsianOption(SimBase):
    ...
    def _calc_payoff(self, underlying_prc, sim_t, **kwargs):
        ...
        prc = max(final_prc - strike, 0) * np.exp(-self.r * sim_t[-1])
        return prc
...
if __name__ == "__main__":
    ...
    sim_t_ = 50 / 365
    eu_opt = EuropeanOption(sim_engine_, sim_times_)
    eu_p = eu_opt.sim_exe(sim_t_, strike=100)
    print(eu_p)

    sim_t_ = np.array([5, 10, 15, 20]) / 365
    as_opt = AsianOption(sim_engine_, sim_times_)
